...
locals {
  ...
  access_key_id_secret_id     = try(data.ns_connection.user-pool.outputs.access_key_id_secret_id, "")
  secret_access_key_secret_id = try(data.ns_connection.user-pool.outputs.secret_access_key_secret_id, "")
}
...
data "aws_secretsmanager_secret_version" "access_key_id" {
  count     = length(local.access_key_id_secret_id) > 0 ? 1 : 0
  secret_id = local.access_key_id_secret_id
}

data "aws_secretsmanager_secret_version" "secret_access_key" {
  count     = length(local.secret_access_key_secret_id) > 0 ? 1 : 0
