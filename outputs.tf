...
output "secrets" {
  value = [
    ...
    },
    {
      ...
      value = try(data.aws_secretsmanager_secret_version.access_key_id[0].secret_string, "")
    },
    {
      name  = "COGNITO_SECRET_ACCESS_KEY"
      value = try(data.aws_secretsmanager_secret_version.secret_access_key[0].secret_string, "")
    }
  ]
}
